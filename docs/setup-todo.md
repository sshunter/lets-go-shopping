...
- [ ] Routine within-constraint upgrades + verification: see [docs/dependency-management.md](dependency-management.md) and run `dart run tool/deps.dart report` / `apply`
- [ ] Native toolchain bumps (Kotlin/AGP/Gradle, iOS deployment target, Flutter SDK, built-in Kotlin): follow the native-bump runbook in that doc; the tool never edits native files
